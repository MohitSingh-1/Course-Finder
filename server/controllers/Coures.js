...
exports.createCourse = async (req, res) => {
    try {
        ...
        // get data
        const { courseName, courseDescription, whatYouWillLearn, price, visitCourse, category } = req.body;
        // get thumbnail
        const thumbnail = req.files.thumbnailImage;

        // validation
        if (!courseName || !courseDescription || !whatYouWillLearn || !price || !visitCourse || !category) {
            return res.json({
                success: false,
        ...
        const imageUploadedDetails = await uploadImageToCloudinary(thumbnail, process.env.FOLDER_NAME);
...
exports.getCourseDetails = async (req, res) => {
    try {
        ...
        const courseDetails = await Course.find({ _id: courseId })
                                            .populate(         // populate the instructor and then you get the user data and then populate additional details of that populated user data
                                                {
                                                    path: "instructor",
                                                    populate: {
